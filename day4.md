...
<p>Your puzzle answer was <code>905</code>.</p>

## Challenges🎯

📍Puzzle 1

- using `slicing `instead of `strip()` and `split()` functions to remove the special characters
- thinking of disjoint to find the contained elements in lists ,eventually using `all` 

📍Puzzle 2

- switching to else can give only not covered elements (so just use `any` )

## Wrap up!🎁

- special characters handling and series sublists in a row is bit confusing ,indexing helps